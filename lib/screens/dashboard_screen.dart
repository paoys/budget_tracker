  ...
  Widget build(BuildContext context) {
    ...
    if (p.totalIncome == 0) {
      return const EmptyState(
          ...
          title: 'No income yet',
          message:
              'Add your income in the Budget tab to get started with budgeting.');
    }
      ...
      children: [
        ...
        if (isWide)
          TwoColumnLayout(
            leftFlex: 1.1,
            rightFlex: 0.9,
            left: _SpendingDonut(p: p),
            right: _BudgetProgressCard(p: p),
          )
        else ...[
          _SpendingDonut(p: p),
          const SizedBox(height: 16),
        ...
        if (p.expenses.isNotEmpty) ...[
          const SizedBox(height: 16),
          _MonthlyBarChart(p: p)
        ],
        if (p.overdueRecurring.isNotEmpty) ...[
          const SizedBox(height: 16),
          _RecurringDueCard(p: p)
        ],
        if (p.cardsDueSoon.isNotEmpty) ...[
          const SizedBox(height: 16),
          _DueSoonCard(cards: p.cardsDueSoon)
        ],
        if (p.goalsNearingDeadline.isNotEmpty) ...[
          const SizedBox(height: 16),
          _GoalsDeadlineCard(p: p)
        ],
        const SizedBox(height: 16),
        _RecentExpenses(p: p),
      ],
    ...
    return ResponsiveCenter(maxWidth: 1100, child: mainContent);
  }
}
...
class _HeroCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final spentPct = p.totalIncome > 0
        ? (p.totalSpent / p.totalIncome).clamp(0.0, 1.0)
        : 0.0;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(monthFmt.format(DateTime.now()),
          ...
          Container(
            ...
            child: Text(isOver ? '⚠ Over budget' : '✓ On track',
                style: GoogleFonts.inter(
                    ...
                    color: isOver ? kDangerColor : Colors.white70)),
          ),
        ]),
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text('${pesoFmt.format(p.totalSpent)} spent',
                style: GoogleFonts.inter(fontSize: 12, color: Colors.white60)),
            Text(
                '${pesoFmt.format(p.remainingTotal.abs())} ${isOver ? "over" : "left"}',
                style: GoogleFonts.inter(
                    fontSize: 12,
                    color: isOver ? kDangerColor : kSuccessColor,
                    fontWeight: FontWeight.w600)),
          ]),
          const SizedBox(height: 8),
          ClipRRect(
            ...
            child: LinearProgressIndicator(
                ...
                valueColor: AlwaysStoppedAnimation(
                    isOver ? kDangerColor : kSuccessColor),
                minHeight: 8),
          ),
        ]),
        const SizedBox(height: 16),
        Row(children: [
          ...
          const SizedBox(width: 8),
          _MiniPill(
              'Savings', p.savingsBudget, p.totalSavingsSpent, kSavingsColor),
        ]),
      ]),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Expanded(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(pesoFmt.format(budget - spent),
            style: GoogleFonts.plusJakartaSans(
                fontSize: 13,
                color: Colors.white,
...
class _ThreeStats extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
        ...
        Expanded(
            child: StatTile(
                label: 'Total Saved',
                value: p.bankAccounts.fold(0.0, (s, a) => s + a.balance),
                accentColor: kSavingsColor,
            ...
            child: Row(children: [
              Expanded(
                ...
                pieTouchData: PieTouchData(
                    touchCallback: (e, res) => setState(() => _touched =
                        res?.touchedSection?.touchedSectionIndex ?? -1)),
                sectionsSpace: 2,
                centerSpaceRadius: 45,
                sections: [
                  if (p.totalNeedsSpent > 0)
                    PieChartSectionData(
                        value: p.totalNeedsSpent,
                        color: colors.needs,
                        title: '',
                        radius: _touched == 0 ? 48 : 40),
                  if (p.totalWantsSpent > 0)
                    PieChartSectionData(
                        value: p.totalWantsSpent,
                        color: colors.wants,
                        title: '',
                        radius: _touched == 1 ? 48 : 40),
                  if (p.totalSavingsSpent > 0)
                    PieChartSectionData(
                        value: p.totalSavingsSpent,
                        color: colors.savings,
                        title: '',
                        radius: _touched == 2 ? 48 : 40),
                ],
              ))),
              ...
              Column(
                  ...
                  children: [
                    _DonutLegend(
                        'Needs', p.totalNeedsSpent, p.totalSpent, colors.needs),
                    const SizedBox(height: 14),
                    _DonutLegend(
                        'Wants', p.totalWantsSpent, p.totalSpent, colors.wants),
                    const SizedBox(height: 14),
                    _DonutLegend('Savings', p.totalSavingsSpent, p.totalSpent,
                        colors.savings),
                  ]),
            ])),
    ]));
...
class _BudgetProgressCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return AppCard(
      ...
      BudgetProgressBar(
          label: 'Needs',
          spent: p.totalNeedsSpent,
          budget: p.needsBudget,
          color: colors.needs),
      const SizedBox(height: 16),
      BudgetProgressBar(
          label: 'Wants',
          spent: p.totalWantsSpent,
          budget: p.wantsBudget,
          color: colors.wants),
      const SizedBox(height: 16),
      BudgetProgressBar(
          label: 'Savings',
          spent: p.totalSavingsSpent,
          budget: p.savingsBudget,
          color: colors.savings),
    ]));
  }
}
            ...
            barTouchData: BarTouchData(
              ...
              touchTooltipData: BarTouchTooltipData(
                getTooltipColor: (touchedBarGroup) =>
                    colors.isDark ? Colors.grey[900]! : Colors.grey[100]!,
            ...
            titlesData: FlTitlesData(
              ...
              bottomTitles: AxisTitles(
                ...
                getTitlesWidget: (v, _) => Padding(
                    ...
                    child: Text(DateFormat('MMM').format(months[v.toInt()]),
                        style: GoogleFonts.inter(
                            fontSize: 10,
        ...
        Row(children: [
          ...
          Text('Recurring Due',
              style: GoogleFonts.inter(
                  ...
                  letterSpacing: 0.5)),
          const Spacer(),
          Text('${due.length} item${due.length > 1 ? "s" : ""}',
              style: GoogleFonts.inter(fontSize: 11, color: kNeedsColor)),
        ]),
                      ...
                      Text(t.title,
                          style: GoogleFonts.inter(
                              ...
                              color: Theme.of(context)
                                  .extension<AppColors>()!
                                  .textPrimary)),
                      Text('${t.frequencyLabel} · ${pesoFmt.format(t.amount)}',
                          style: GoogleFonts.inter(
                              fontSize: 11,
                              color: Theme.of(context)
                                  .extension<AppColors>()!
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        border:
                            Border.all(color: kNeedsColor.withOpacity(0.3))),
                    child: Text('Skip',
                        style: GoogleFonts.inter(
                            fontSize: 12,
                            color: kNeedsColor,
                ...
                GestureDetector(
                  ...
                  child: Container(
                    padding:
                        const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
                    decoration: BoxDecoration(
                        color: kNeedsColor.withOpacity(0.15),
                        borderRadius: BorderRadius.circular(8)),
                    child: Text('Log Now',
                        style: GoogleFonts.inter(
                            fontSize: 12,
                    ...
                    Column(
                        ...
                        children: [
                          ...
                          Text(
                            ...
                            style: GoogleFonts.inter(
                                fontSize: 11, color: kWarningColor),
                          ),
                        ]),
...
class _ExpenseTile extends StatelessWidget {
    ...
    return Padding(
      ...
      child: Row(children: [
        Container(
          ...
          height: 38,
          decoration: BoxDecoration(
              color: c.withOpacity(colors.isDark ? 0.15 : 0.1),
              borderRadius: BorderRadius.circular(10)),
              ...
              Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: const BoxDecoration(
                        color: kNeedsColor, shape: BoxShape.circle),
                    child:
                        const Icon(Icons.repeat, size: 7, color: Colors.white),
                  )),
          ]),
        ),
        ...
        Expanded(
          ...
          Row(children: [
            ...
            const SizedBox(width: 6),
            Text(shortDateFmt.format(expense.date),
                style:
                    GoogleFonts.inter(fontSize: 11, color: colors.textMuted)),
          ]),
        ])),
        Text(pesoFmt.format(expense.amount),
            style: GoogleFonts.plusJakartaSans(
                fontSize: 14,
                fontWeight: FontWeight.w700,
                color: colors.textPrimary)),
  ...
  IconData _catIcon(CategoryType t) => t == CategoryType.needs
      ? Icons.home_outlined
      : t == CategoryType.wants
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (goals.length > 3)
          Text('+${goals.length - 3} more goals',
              style: GoogleFonts.inter(fontSize: 11, color: colors.textMuted)),
      ]),
    );
  }
